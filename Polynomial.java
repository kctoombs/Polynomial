...
/**
 * <p>
 * Polynomial represents a polynomial from algebra with integer coefficients.
 * e.g. 4x<sup>3</sup> + 3x<sup>2</sup> - 5x + 2
 * </p>
 * 
 * <p>
 * Polynomial extends ArrayList<Integer>, and contains the coefficients of the
 * polynomial, such that for Polynomial p, p.get(i) returns the coefficient of
 * the x<sup>i<sup> term. p.get(0) returns the constant term.
 * </p>
 * 
 * <p>
 * For a Polynomial object p that represents a polynomial of degree d, (i.e.
 * where x<sup>d</sup> is the highest order term with a non-zero coefficient),
 * invoking p.get(k) where k&gt;d should result in an IndexOutOfBoundsException;
 * that is, the ArrayList should be of size exactly d+1.
 * </p>
 * 
 * @author P. Conrad
 * @author Kenneth Toombs
 * @version UCSB, CS56, W16
 */

public class Polynomial extends ArrayList<Integer> {

	/**
	 * Assign Polynomial.debug to true or false to turn debugging output on or
	 * off.
	 */
	...
	/**
	 * Construct polynomial from int array of coefficients. The coefficients are
	 * listed in order from highest to lowest degree. The degree is the size of
	 * the array - 1.
	 * 
	 * 
	 ...
	 * 
	 * NOTE that the order of coefficients is not necessarily the way they will
	 * be stored in the array. That is, the order of coefficients in the array
	 * passed in is from highest degree down to lowest degree, so for a cubic:
	 * <ul>
	 ...
	 * </ul>
	 * 
	 * It is done this way so that when initializing a polynomial from an array
	 * literal, the order of coefficients mirrors the way polynomials are
	 * typically written, from highest order term to lowest order term:
	 * 
	 * Example: to represent 4x<sup>3</sup> - 7x<sup>2</sup> + 5,<br />
	 * use: <code>Polynomial p = new Polynomial (new int [] {4,-7,0,5});</code>
	 * <br />
	 * NOT: <code>Polynomial p = new Polynomial (new int [] {5,0,-7,4});</code>
	 * 
	 * 
	 * @param coeffsHighToLow
	 *            array of coefficients in order from largest degree down to
	 *            smallest. If array has length zero, return a polynomial of
	 *            degree zero with constant value zero.
	 */
	 ...
	 * 
	 * Leading coefficient has negative sign in front, with no space. Other
	 * signs have a space on either side. Coefficients that are ones should be
	 * omitted (except in the x^0 term). Terms with zero coefficients, except in
	 * the special case where the polynomial is of degree zero, and the constant
	...
	public String toString() {
	    StringWriter sw = new StringWriter(); /* Declare StringWriter object to concatenate signs, powers, and integers.*/
		int size = this.size();
		if (this.size() == 1) {
			sw.append(String.valueOf(this.get(0)));
		}

		else if (this.size() == 2) {
			if (this.get(1) > 0) {
				if (this.get(1) != 1) {
					sw.append(String.valueOf(this.get(1)));
				}
				sw.append("x");
			}
			if (this.get(1) < 0) {
				sw.append("-");
				if (this.get(1) != -1)
					sw.append(String.valueOf(Math.abs(this.get(1))));
				sw.append("x");
			}

			if (this.get(0) < 0) {
				sw.append(" - ");
				sw.append(String.valueOf(Math.abs(this.get(0))));
			} else if (this.get(0) > 0) {
				sw.append(" + ");
				sw.append(String.valueOf(this.get(0)));
			}
		}
			...
			for (int i = size - 1; i >= 0; i--) {
				if (i == size - 1) {
					if (this.get(i) > 0) {
						if (this.get(i) != 1) {
							sw.append(String.valueOf(this.get(i)));
							sw.append("x");
							sw.append("^");
							sw.append(String.valueOf(i));
					...
					if (this.get(i) != 0) {
						if (this.get(i) > 0) {
							...
							sw.append(" - ");
							sw.append(String.valueOf(Math.abs(this.get(i))));
						}
	...
	/**
	 * Construct polynomial from string representation that matches the output
	 * format of the Polynomial toString method.
	 * 
	...
	public Polynomial(String s) {
		...
		Pattern integerConstantPattern = Pattern.compile("^-?\\d+$");
		...
		Pattern degreeOnePattern = Pattern.compile("^(-?)(\\d*)x( ([+-]) (\\d+))?$");
		// Explanation:
		// start/end ^ $
		...
		String twoOrMoreRe = "^" // start of the string
				+ "([-]?)(\\d*)x\\^(\\d+)" // first x^d term, groups 1,2,3
				+ "(( [+-] \\d*x\\^\\d+)*)" // zero or more x^k terms group 4
											// (and 5)
		...
		if (degreeTwoOrMoreMatcher.matches()) {
			...
			if (optionalXTermPart != null && !optionalXTermPart.equals("")) {
				...
				if (debug) {
					System.out.println("optionalXTermPart=" + optionalXTermPart);
				}
	...
	/**
	 * determine whether two polynomials are equal (same degree, same
	 * coefficients)
	 * 
	 * @param o
	 *            arbitrary object to test for equality
	...
	public boolean equals(Object o) {
		...
		// Check the size of each ArrayList.
		// If they don't match, return false
		if (this.size() != p.size())
			return false;

		// If the sizes match, check whether the
		...
		return true; // Return true if sizes and values match.

	}

	/**
	 * Given an int [] of coefficients from lowest to highest degree (where the
	 * index in the array matches the power of the x term), find the degree of
	 * the polynomial (ignoring trailing terms with a coefficient of zero) If
	 * all terms are zero, return 0.
	 * 
	 * This is a utility method that may be useful in converting between the low
	 * to high and high to low representations of coefficients, both in user
	 * programs that use the Polynomial class, as well as in internal routines
	 * used to implement Polynomial methods.
	 * 
	 * @param coeffsLowToHigh
	 *            coefficients of a polynomial in order from lowest degree to
	 *            highest degree. May have trailing zeros.
	 * @return the degree of the polynomial as an int, ignoring trailing terms
	 *         with a coefficient of zero
	 * 
	 */

	public static int degreeCoeffsLowToHigh(int[] coeffsLowToHigh) {
		int degree = coeffsLowToHigh.length - 1;
	...
	/**
	 * Given an int [] of coefficients from highest to lowest degree (the formal
	 * used for input to the Polynomial constructor), find the degree of the
	 * polynomial (ignoring leading terms with a coefficient of zero) If all
	 * terms are zero, return 0.
	 * 
	 * This is a utility method that may be useful in converting between the low
	 * to high and high to low representations of coefficients, both in user
	 * programs that use the Polynomial class, as well as in internal routines
	 ...
	 * 
	 * @param coeffsHighToLow
	 *            coefficients of a polynomial in order from highest degree
	 *            first to lowest degree last. May have leading zeros.
	 * @return the degree of the polynomial as an int, ignoring leading terms
	 *         with a coefficient of zero
	 * 
	 */
	...
	public static int degreeCoeffsHighToLow(int[] coeffsHighToLow) {
		...
		for (int i = 0; i < (coeffsHighToLow.length - 1); i++) {
	...
	/**
	 * Convert a list of coefficients in order from highest degree to lowest
	 * degree (the order used for input to the Polynomial constructor) to one
	 * where the order is lowest degree to highest degree (where index matches
	 * power of the x term).
	 * 
	 * @param coeffsHighToLow
	 *            coefficients of a polynomial in order from highest degree to
	 *            lowest degree. May have leading zeros.
	 * @return An int [] with coefficients in order from lowest degree to
	 *         highest degree. No trailing zeros.
	 */

	public static int[] lowToHigh(int[] coeffsHighToLow) {
		int degree = degreeCoeffsHighToLow(coeffsHighToLow);
		int size = degree + 1;
		int length = coeffsHighToLow.length - 1;
		int[] intArray = new int[size];
		for (int i = 0; i < size; i++) {
			intArray[i] = coeffsHighToLow[length];
			length--;
		}
		return intArray;
	}
	...
	/**
	 * Convert a list of coefficients in order from lowest degree to highest
	 * degree (where index matches power of the x term) to one in order from
	 * highest degree to lowest degree (the order used for input to the
	 * Polynomial constructor).
	 * 
	 * @param coeffsLowToHigh
	 *            coefficients of a polynomial in order from lowest degree to
	 *            highest degree. May have trailing zeros.
	 * @return An int [] with coefficients in order from highest degree to
	 *         lowest degree. No leading zeros.
	 */

	public static int[] highToLow(int[] coeffsLowToHigh) {
		int degree = degreeCoeffsLowToHigh(coeffsLowToHigh);
		int size = degree + 1;
		// int length = coeffsLowToHigh.length - 1;
		int[] intArray = new int[size];
		for (int i = 0; i < degree + 1; i++) {
			intArray[i] = coeffsLowToHigh[size - 1];
			size--;
		}
		return intArray;
	}
	...
	/**
	 * return a new Polynomial which has as its value the this polynomial plus
	...
	/**
	 * return a new Polynomial which has as its value the this polynomial times
	 * the one passed in as a parameter.
	 * 
	 * @param p
	 *            the Polynomial to multiply this one by
	 * @return product of this Polynomial and p
	 * 
	 */

	public Polynomial times(Polynomial p) {
		int realSize = this.getDegree() + p.getDegree() + 1;
		int x = 0;
		...
		for(int i = 0; i < this.size(); i++){
			for(int j = 0; j < p.size(); j++){ 
				result[i+j] = (this.get(i) * p.get(j) + result[i+j]);		
			}
		}	
		...
		Polynomial p_final = new Polynomial(temp);
		return p_final;

	}

	/**
	 * return a new Polynomial which has as its value the this polynomial minus
	 * the one passed in as a parameter.
	 * 
	 * @param p
	...
	public Polynomial minus(Polynomial p) {
		Polynomial p1 = new Polynomial();
		if (p.size() == this.size()) {
			p1.set(0, (this.get(0) - p.get(0)));
			for (int i = 1; i < this.size(); i++)
				p1.add(this.get(i) - p.get(i));
		} else if (p.size() >= this.size()) {
			p1.set(0, (this.get(0) - p.get(0)));
			for (int i = 1; i < this.size(); i++) {
				p1.add((this.get(i) - p.get(i)));
	...
	/**
	 * Main for testing constructing Polynomials from strings, and for testing
	 * plus, minus and times.
	 * 
	 * At Unix command line, use single quotes to make the entire Polynomial be
	 * a single argument, and use \* when operating is * (to avoid having *
	 * expanded as a wildcard.) For example:
	...
	public static void main(String[] args) {
		...
		if (args.length != 3) {
			System.out.println("There should be either 1 cmd line argument or 3 arguments, but there were: "
		...
		if (args[1].equals("+")) {
			...
			System.out.println("(" + p.toString() + ") + (" + p2.toString() + ") = " + result.toString());
